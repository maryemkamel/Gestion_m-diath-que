...
public class Administrateur extends JFrame implements ActionListener {
	...
	public Administrateur(){
		...
		add(pn);
		//titre
				lbtitre=new JLabel("Administrateur");
	...
	public static void main(String[] args) {
		Administrateur cd=new Administrateur();
	    cd.setVisible(true);
	...
	public void actionPerformed(ActionEvent e) {
		// Passage a l'interface de l'espace administrateur 
		String log=tf1.getText();
		//char[] mdpc=;
		String mdp= new String(tf2.getPassword());
		if(e.getSource()==bt){
			String qr="SELECT * from `bibliotheque`.`administrateur` where `login`='"+log+"' and `motDePasse`='"+mdp+"';";
			 try{
				 ...
				 if(résultats.next()){
					 JOptionPane.showMessageDialog(null,"Vous êtes connecté");
					   dispose();
						EspaceAdmin ab=new EspaceAdmin();
						ab.setVisible(true);
