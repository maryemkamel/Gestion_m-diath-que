...
public class Créerunjournal extends JFrame implements ActionListener{
...
Créerunjournal(){
setTitle("ESPACE CREATION JOURNAL :");
setSize(700, 500);
...
contenu.setBorder(new TitledBorder("CREER UN JOURNAL "));
add(contenu);
...
Créerunjournal cr= new Créerunjournal();
cr.setVisible(true);
}
...
public void actionPerformed(ActionEvent e) {
	...
	if(e.getSource()==btnAjouter) {
		try {
			...
			PreparedStatement ps=conction.prepareStatement("INSERT INTO `bibliotheque`.`document`(`type`,`titre`,`date`) VALUES('JOURNAL',?,?);");
			ps.setString(1, ttitre.getText());
			ps.setDate(2, java.sql.Date.valueOf(tdate.getText()));
			ps.execute();
			conction.close();
			JOptionPane.showMessageDialog(null,"Journal crée" );
			}
			
		 }catch(Exception ev){
			JOptionPane.showConfirmDialog(null,"Journal non crée verifier format date yyyy-mm-dd  ");
